...
class TestSignin():
    ...
    def test_empty_values(client):
        ...
        response=client.post("/signin",data=json.dumps(dict(email="", password=TestSignin.password)), content_type="application/json")
    ...
    def test_valid_keys(client):
        ...
        response=client.post("/signin",data=json.dumps(dict(username=TestSignin.email, password=TestSignin.password)), content_type="application/json")
        data=json.loads(response.data)
        assert response.status_code==400
    ...
    def test_email_password(client):
        ...
        response=client.post("/signin",data=json.dumps(dict(email=1234, password=TestSignin.password)), content_type="application/json")
        data=json.loads(response.data)
        assert response.status_code==400
        assert data["error"] == "Please provide email and password as valid strings values."
    
    @staticmethod
    def test_email_exisitance(client):
        """ Test if user sigining in is a registered user """
        response=client.post("/signin",data=json.dumps(dict(email='njayaandrew@yahoo.com', password=TestSignin.password)), content_type="application/json")
        data=json.loads(response.data)
    ...
    def test_space_characters(client):
        ...
        response=client.post("/signin",data=json.dumps(dict(email='  ', password='    ')), content_type="application/json")
        data=json.loads(response.data)
        assert response.status_code==401
